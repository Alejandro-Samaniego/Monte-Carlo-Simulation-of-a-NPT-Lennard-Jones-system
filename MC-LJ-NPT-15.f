...
c     3. Reading initial configuration (positions, velocities) in A and A/ps
      ...
      open(2,file='last_config_P_15.dat',status='old') ! We read the last configuration of the last rho (if we want to calculate rho=0.7, we use last conf of rho=0.8)
      do is = 1,natoms
         read(2,*) (r(l,is),l=1,3)
...
c
      open(3,file='thermodynamics_P_15.dat',status='unknown')
      open(4,file='gr_P_15.dat',status='unknown')
      open(5,file='last_config_P_15.dat',status='unknown')
