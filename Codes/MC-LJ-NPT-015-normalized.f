...
c     3. Reading initial configuration (positions, velocities) in A and A/ps
      ...
      open(2,file='last_config_P_015.dat',status='old') ! We read the last configuration of the last rho (if we want to calculate rho=0.7, we use last conf of rho=0.8)
      do is = 1,natoms
         read(2,*) (r(l,is),l=1,3)
...
c
      open(3,file='thermodynamics_P_015_norm.dat',status='unknown')
      open(4,file='gr_P_015_norm.dat',status='unknown')
      open(5,file='last_config_P_015_norm.dat',status='unknown')
...
c     PSEUDO CODI NPT
     ...
     &               ,nchange) !Attempts to change the volume
        ...
        if (mod(icycl,nsamp).eq.0) then
          call sample(natoms,r,boxlength,rc,epot,gaux,
     &                delg,ppot) !! sample averages
          ...
          do i=1,100  !100 = nhis = total number of bins
            ...
            gaux(i)=0
          enddo
          write(3,*) icycl/nsamp, ppot, epot, rho, boxlength
        endif
      enddo
      ...
      write(3,*) naccepted,nacceptedvol,rho,ncountervol,0
      write(3,*) ncounttest,ndumped,boxlength,0,0
      do jj = 1, 100
        g(jj)=g(jj)/nconf
      enddo
