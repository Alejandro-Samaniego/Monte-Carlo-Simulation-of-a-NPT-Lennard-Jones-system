...
c
      open(3,file='thermodynamics_P.dat',status='unknown')
      open(4,file='gr_P.dat',status='unknown')
      open(5,file='last_config_P.dat',status='unknown')
...
c     PSEUDO CODI NPT
     ...
     &                delg,ppot) !! sample averages
          ...
          ppot=ppot+rho*2.d0
          write(3,*) icycl/nsamp, ppot, epot, boxlength*sigma
        endif
      enddo
      ...
      write(3,*) naccepted,nacceptedvol,rho,ncountervol
      write(3,*) ncounttest,ndumped,0,0
      close(3)
      close(4)
      close(5)
      end
